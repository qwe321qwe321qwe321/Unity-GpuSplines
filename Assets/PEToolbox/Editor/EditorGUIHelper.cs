...
namespace PeDev {
	...
	internal static class EditorGUIHelper {
		...
		public static void DrawSprite(Rect position, Sprite sprite, bool keepAspect = false) {
			if (sprite == null) { return; }
			// trying to load preview texture.
			Texture tex = AssetPreview.GetAssetPreview(sprite);
			// Check if it is loading.
			// from: https://gamedev.stackexchange.com/questions/159508/how-to-get-image-icon-texture-of-asset-from-project-window
			if (AssetPreview.IsLoadingAssetPreview(sprite.GetInstanceID())) {
				return;
			}
			Assert.IsTrue(tex, "I don't know why Unity doesn't load this sprite's preview.");

			EditorGUI.DrawTextureTransparent(position, tex, keepAspect ? ScaleMode.ScaleToFit : ScaleMode.StretchToFill);

			// Render the sprite by GUI.DrawTextureWithTexCoords with coordinates.
			...
			//		float oriHeight = position.height;
			//		position.height *= (float)sprite.texture.height / sprite.texture.width;
			//		position.y -= (position.height - oriHeight) / 2;
			//	} else {
			//		float oriWidth = position.width;
			//		position.width *= (float)sprite.texture.width / sprite.texture.height;
			//		position.x -= (position.width - oriWidth) / 2;
			//	}
			//}
